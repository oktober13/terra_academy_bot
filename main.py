...
async def handle_users_command(message: types.Message):
    ...
    admin_ids = [638570932, 366856617]  # Замените на ваши собственные идентификаторы администраторов

    # Проверка, что сообщение отправлено вам (администратору)
    if message.from_user.id in admin_ids:
        # Извлечение списка пользователей из базы данных SQLite
        cursor.execute("SELECT username, user_id FROM users")
        users = cursor.fetchall()

        # Форматирование списка пользователей для вывода
        user_list = "\n".join([f"{username}: {user_id}" for username, user_id in users])

        # Отправка списка пользователей в качестве ответа на команду
